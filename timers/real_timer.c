...
static void displayTimes(const char* msg, Boolean includeTimer)
{
    ...
    if (includeTimer) {
        if (getitimer(ITIMER_REAL, &itv) == -1)
            errExit("getitimer");
        printf(" %6.2f %6.2f\n", itv.it_value.tv_sec + itv.it_value.tv_usec / 1000000.0, itv.it_interval.tv_sec + itv.it_interval.tv_usec / 1000000.0);
    }
...
int main(int argc, char const *argv[])
{
    ...
    itv.it_value.tv_usec = (argv > 2) ? getLong(argv[2], 0, "usecs") : 0;
    ...
    while (true) {
        while (((clock() - prevClock) * 10 / CLOCK_PER_SEC) < 5) {
            if (gotAlarm) {
                gotAlarm = 0;
